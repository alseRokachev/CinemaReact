...
const SpecialForYouCarousel = () => {
    const dispatch = useDispatch()
    const data = useSelector(state => state.filmsStore.films)
    ...
    return (
        <div className={'mt-10 mb-24 w-full h-72'}>
            <div className={'w-full h-full flex overflow-x-auto genre_scrollbar pb-8'}>
                {data.map((film, index) => {
                    return <div
                        className={'min-w-[300px] max-h-[360px] w-full h-full bg-slate-800 rounded-3xl mr-7 flex items-start justify-start text-2xl text-slate-50 relative duration-500 hover:cursor-pointer hover:bg-slate-700'}
                        key={index}>
...
export default SpecialForYouCarousel;